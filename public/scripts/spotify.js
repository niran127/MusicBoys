...
async function getSpToken() {
  ...
  if (localStorage.getItem("sp_at") && Date.now() < exp - 60000)
...
// voor guests (zonder login) - gebruikt server proxy om secret veilig te houden
...
function setFollowedArtists(artists) {
...
window._initSpotifyPlayer = async () => {
  ...
  spPlayer.addListener("ready", ({ device_id }) => {
    ...
    wirePlayerControls();
    // Start live progressbalk zodra player klaar is
    clearInterval(spInterval);
    spInterval = setInterval(async () => {
      const st = await spPlayer?.getCurrentState();
      if (st) updatePlayerBar(st);
    }, 1000);
    console.log("[MusicBoys] Spotify klaar op device:", device_id);

    // Automatisch overzetten naar dit device
...
async function playSong(trackUri, meta, isNavAction = false) {
  ...
  if (meta) {
    ...
    const isLiked = getLikes().some(
      (l) => window.getTrackId(l.uri) === window.getTrackId(singleUri),
    );
...
function loadPlayerFromStorage() {
  ...
  if (current.uri && current.meta) {
    ...
    const isLiked = getLikes().some(
      (l) => window.getTrackId(l.uri) === window.getTrackId(current.uri),
    );
    const btn = document.getElementById("like-btn");
    if (btn) btn.classList.toggle("liked", isLiked);
  }
...
async function spotifyFollowArtist(artistId) {
  const token = await getSpToken();
  if (!token) return;
  ...
  if (res.ok) {
    // 2. Ook in onze DB opslaan
    try {
      const info = await spotifyGetArtist(artistId);
      ...
      console.error("Fout bij opslaan artist in DB:", err);
    }
  }
...
document.addEventListener("keydown", (e) => {
  // Voorkom actie als we in een input veld typen
  if (
