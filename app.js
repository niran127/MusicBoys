...
app.use(session({
    ...
    cookie: { 
        ...
        httpOnly: true
        // geen maxAge: sessie verloopt zodra browser sluit (tenzij 'onthouden' is aangevinkt)
    }
}));

// Auth Middleware: enforces real authentication so every user has their own secure data
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (!user) return res.render('login', { error: 'Gebruikersnaam of wachtwoord is incorrect' });
        
        const isMatch = await bcrypt.compare(password, user.password);
        if (!isMatch) return res.render('login', { error: 'Gebruikersnaam of wachtwoord is incorrect' });
        ...
        // anders: sessiecookie, vervalt bij sluiten browser
...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        if (existing) return res.render('register', { error: 'Gebruikersnaam is al bezet' });
...
app.get('/', requireAuth, async (req, res) => {
    try {
        ...
        // Laad uitsluitend de playlists die horen bij DIT userId
...
// API Routes voor User / Sync
app.post('/api/user/sync', requireAuth, async (req, res) => {
  try {
    const { id, name } = req.body; // Spotify info
    const user = await User.findById(req.session.userId);
...
// laatst afgespeelde nummer opslaan
...
app.get('/api/user/likes', requireAuth, async (req, res) => {
  try {
    ...
    // terugwaartse compatibiliteit voor oude records zonder meta
    likes = likes.map(t => {
...
app.get('/api/user/playlists', requireAuth, async (req, res) => {
  try {
    ...
    // filter oude nummers zonder meta object
...
app.post('/api/playlists', requireAuth, async (req, res) => {
  try {
    ...
    // beveilig playlist creatie aan database sessie
...
app.post('/api/user/game-score', requireAuth, async (req, res) => {
  try {
    ...
    // Future expansion: we can add highscore persistence
    // For now we just accept it to show backend integration.
    res.status(200).send();
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
