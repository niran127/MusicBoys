...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      (user.likedTracks as any) = user.likedTracks.filter(
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      (user.followedArtists as any) = user.followedArtists.filter(
        (a: any) => a.id !== req.params.artistId,
      );
      await user.save();
      res.json(user.followedArtists);
    } catch (err) {
...
router.get(
  ...
  requireAuth,
  async (req: Request, res: Response): Promise<void> => {
    const term = (req.query.term as string) || "pop";
